...
export default function Perfil() {
  ...
  <h2>Figures Postadas</h2>

  {produtosPostados.length === 0 ? (
    <p>Nenhuma figure postada ainda.</p>
  ) : (
    <div className="lista-produtos">
      {produtosPostados.map(figura => (
        <div key={figura.id} className="card-produto" onClick={() => navigate(`/figura/${figura.id}`)}>
          <img src={figura.imagem_url} alt={figura.nome} className="imagem-produto" />
          <h3>{figura.nome}</h3>
